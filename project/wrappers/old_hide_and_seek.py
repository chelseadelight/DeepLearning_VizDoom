...
class HideAndSeekWrapper(gym.Wrapper):
    def __init__(self, env):
        ...
        print(">>> Initializing HideAndSeekWrapper")
        print("self.unwrapped:", self.unwrapped)
    ...
    def get_role(self) -> str:
        # get curr_policy_idx from unwrapped env
        self_env = self.unwrapped
        curr_policy_idx = getattr(self_env, 'curr_policy_idx', 0)

        if curr_policy_idx % 2 == 0:
            return "seek"
        else:
            print("Agent role: hide")
            return "hide"
    ...
    def step(self, action: ActType) -> tuple[ObsType, SupportsFloat, bool, bool, dict[str, Any]]:
        ...
        for label in state.labels:
            if label.object_category == "Self":
                player = Player(label.object_position_x, label.object_position_y, label.object_angle)
            elif label.object_category == "Player":
        ...
        if self.role == "hide":
            # reward is higher when enemies are farther away, and when they are looking away.
            for distance, own_angle, enemy_angle in metrics:
                if enemy_angle < math.pi / 2:
                    reward -= 1.0 / (distance + 1.0)

        elif self.role == "seek":
            # reward is higher when enemies are closer, and when the player is looking at them.
            for distance, own_angle, enemy_angle in metrics:
                if own_angle < math.pi / 2:
                    reward += 1.0 / (distance + 1.0)
